...
class App extends Component {
    ...
    resetPayment() {
        ...
        if (
            hasNewPaymentState === true &&
            (hasNewPurchasePriceState === false ||
                hasNewInterestState === false ||
                hasNewTermState === false)
        ) {
    ...
    calcPayment() {
        ...
        return (
            (principal *
                ((interest / 1200) *
                    Math.pow(1 + interest / 1200, term * 12))) /
            (Math.pow(1 + interest / 1200, term * 12) - 1)
        ).toFixed(2);
    }
    ...
    calcPrincipalPercent() {
        return (
            ...
            (1 - this.state.downPayment / 100).toFixed(2)
        );
    }
    ...
    handleTermInput = event => this.setState({ term: event.target.value });
